...
function getTimeDiffUntilNowString(date) {
  ...
  const numberHoursLive = String(
    new Date(currentTime - new Date(date).getTime()).getUTCHours()
  ).padStart(2, "0");
  const numberMinutesive = String(
    new Date(currentTime - new Date(date).getTime()).getUTCMinutes()
  ).padStart(2, "0");
  const numberSecondsLive = String(
    new Date(currentTime - new Date(date).getTime()).getUTCSeconds()
  ).padStart(2, "0");
...
const ElementCreator = {
  ...
  createTooltip: function (streamContainer, stream) {
    streamContainer.querySelector("#tooltip-stream-title").textContent =
      stream.title;
    streamContainer.querySelector("#tooltip-stream-started-at").textContent =
      getTimeDiffUntilNowString(stream.started_at);
    streamContainer.querySelector("#tooltiptext").dataset.startedAt =
      stream.started_at;
  },
